...
export const needsScrolling = (scandals) => {
    return scandals.length > 1;
};
// Detect significant gaps in timeline
export const detectTimelineGaps = (scandals) => {
    if (scandals.length <= 1)
        return [];
    const sortedScandals = [...scandals].sort((a, b) => new Date(a.startDate).getTime() - new Date(b.startDate).getTime());
    const gaps = [];
...
export const calculateOptimizedTimelineWidth = (scandals, startYear, endYear, zoomLevel) => {
...
export const calculateTimelineWidth = (startYear, endYear, zoomLevel) => {
    const yearSpan = endYear - startYear;
    return Math.max(1400, yearSpan * BASE_PIXELS_PER_YEAR * zoomLevel);
};
export const getXPositionForDate = (dateString, startYear, endYear, timelineWidth) => {
...
export const calculateOptimizedScandalPositions = (scandals, gaps) => {
    if (scandals.length === 0)
        return [];
    if (scandals.length === 1) {
...
export const calculateScandalPositions = (scandals, startYear, endYear, timelineWidth) => {
    if (scandals.length === 0)
        return [];
...
export const generateYearMarkers = (startYear, endYear, timelineWidth, zoomLevel) => {
    const markers = [];
    const yearSpan = endYear - startYear;
...
export const getVisibleScandals = (positions, scrollLeft, viewportWidth) => {
...
export const getCumulativeScandals = (positions, scandals, scrollLeft, viewportWidth) => {
    ...
    const sortedPositions = cumulativePositions.sort((a, b) => a.date.getTime() - b.date.getTime());
    ...
    const cumulativeScandals = sortedPositions
        ...
        .filter((scandal) => scandal !== undefined);
 ...
 */
export const calculateDynamicDateRange = (scandals) => {
    if (!scandals.length) {
...
export const calculateCurrentYear = (scrollPosition, viewportWidth, timelineWidth, startYear, endYear) => {
