...
const StatsPanel = ({ stats, scandals, onClose }) => {
    ...
    return (<motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'tween', duration: 0.2 }} className="w-80 bg-white dark:bg-gray-800 border-l border-gray-200 dark:border-gray-700 flex flex-col h-full shadow-2xl">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 bg-white dark:bg-gray-800">
          ...
          <div className="space-y-3">
            ...
            return (<div key={category}>
                    ...
                    <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                      <div className={`h-full transition-all duration-300 ${gradientClass}`} style={{ width: `${(count / maxByType) * 100}%` }}/>
                    </div>
                  </div>);
        })}
          </div>
        ...
        {/* By Party */}
        {Object.keys(stats.byParty).length > 0 && (<div>
            <h3 className="text-sm font-medium text-gray-900 dark:text-white mb-3 flex items-center gap-2">
            ...
            <div className="space-y-3">
              {Object.entries(stats.byParty)
                ...
                .map(([party, count]) => (<div key={party}>
                  ...
                  <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                    <div className="h-full bg-red-500 transition-all duration-300" style={{ width: `${(count / maxByParty) * 100}%` }}/>
                  </div>
                </div>))}
            </div>
          </div>)}
        ...
        <div>
          ...
          <div className="space-y-3">
            ...
            .map(([decade, count]) => (<div key={decade}>
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-sm text-gray-700 dark:text-gray-300">
                      {decade}s
                    </span>
                  ...
                  <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                    <div className="h-full bg-green-500 transition-all duration-300" style={{ width: `${(count / maxByDecade) * 100}%` }}/>
                  </div>
                </div>))}
          </div>
        </div>
