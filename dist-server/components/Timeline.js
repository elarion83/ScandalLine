...
import { ZoomIn, ZoomOut, Filter, BarChart3, HelpCircle } from 'lucide-react';
import { AnimatePresence } from 'framer-motion';
import { filterScandals, calculateStats } from '../utils/scandalUtils';
import { filterTimelineBy } from '../utils/contextualFilters';
import { calculateTimelineWidth, calculateScandalPositions, TIMELINE_Y, CARD_WIDTH, getVisibleScandals, getCumulativeScandals, calculateOptimizedTimelineWidth, calculateOptimizedScandalPositions, calculateDynamicDateRange, calculateCurrentYear } from '../utils/timelineLayout';
...
const Timeline = ({ scandals }) => {
    ...
    const { start: startYear, end: endYear } = useMemo(() => calculateDynamicDateRange(filteredScandals), [filteredScandals]);
    // Update filters with new date range
    useEffect(() => {
        // Ne pas mettre à jour les dates si les filtres ont changé à cause d'une case à cocher
        const hasOnlyCheckboxChanges = state.filters.types.length > 0 ||
    ...
    const handleContextualFilterChange = useCallback((filter) => {
    ...
    return (<div className="flex flex-col h-full">
      {/* Main Header */}
      {!state.contextualFilter ? (<div className="texture-overlay bg-gradient-to-r from-gray-800 to-gray-900 text-white px-6 py-4 border-b border-gray-700">
      ...
      <DynamicStatsBar visibleScandals={cumulativeScandals} totalScandals={filteredScandals.length} scrollProgress={scrollProgress} startYear={startYear} endYear={endYear} scrollPosition={state.scrollPosition} viewportWidth={state.viewportWidth} timelineWidth={timelineWidth} onScrollChange={(newPosition) => {
            if (containerRef.current) {
      ...
      <div className="flex flex-1 overflow-hidden relative">
          <div className={`timeline-controls ${showOnboarding ? 'tour-highlight' : ''} ${state.showFilters ? 'filters-open' : ''} ${state.showStats ? 'stats-open' : ''}`} style={{ zIndex: 9999 }}>
            ...
            <button onClick={() => dispatch({ type: 'TOGGLE_FILTERS' })} data-tour="filters" className={`p-2 rounded-lg transition-colors relative ${state.showFilters
            ? 'bg-blue-500 text-white dark:bg-blue-600'
            : 'bg-blue-100 text-blue-600 dark:bg-blue-900 dark:text-blue-300 hover:bg-blue-200 dark:hover:bg-blue-800'}`} title="Filtres">
              <Filter className="w-5 h-5"/>
              {hasActiveFilters && !state.showFilters && (<div className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full border-2 border-white dark:border-gray-800"></div>)}
            </button>
            
            <button onClick={() => dispatch({ type: 'TOGGLE_STATS' })} data-tour="stats" className={`p-2 rounded-lg transition-colors ${state.showStats
            ? 'bg-green-500 text-white dark:bg-green-600'
            : 'bg-green-100 text-green-600 dark:bg-green-900 dark:text-green-300 hover:bg-green-200 dark:hover:bg-green-800'}`} title="Statistiques">
              <BarChart3 className="w-5 h-5"/>
            </button>

            <div className="w-px h-6 bg-gray-200 dark:bg-gray-600 self-center mx-1"></div>
            
            <button onClick={() => setShowOnboarding(true)} className="p-2 rounded-lg transition-colors bg-purple-100 text-purple-600 dark:bg-purple-900 dark:text-purple-300 hover:bg-purple-200 dark:hover:bg-purple-800" title="Aide">
        ...
        {/* Filters Panel */}
        <AnimatePresence>
          {state.showFilters && (<FilterPanel filters={state.filters} onFiltersChange={(filters) => dispatch({ type: 'SET_FILTERS', payload: filters })} scandals={contextuallyFilteredScandals} onClose={() => dispatch({ type: 'TOGGLE_FILTERS' })}/>)}
        </AnimatePresence>

        {/* Main Timeline */}
        <div className="flex-1 flex flex-col min-w-0 relative h-full">
          {/* Timeline Container */}
          <div ref={containerRef} className={`relative flex-1 overflow-x-auto overflow-y-scroll ${state.isTransitioning ? 'opacity-0' : 'opacity-100'} transition-opacity duration-150`} style={{
            height: '100%',
            ...
            <div className="relative timeline-container" style={{
            width: timelineWidth,
            minHeight: '800px',
            paddingBottom: '100px'
        }}>
            ...
            transition: 'left 150ms ease'
                  ...
                  <span className="text-[20rem] font-bold text-gray-100 dark:text-gray-800/50 opacity-80">
                    {calculateCurrentYear(state.scrollPosition, state.viewportWidth, timelineWidth, startYear, endYear)}
                  </span>
                </div>

                <TimelineAxis startYear={startYear} endYear={endYear} timelineWidth={timelineWidth} zoomLevel={state.zoomLevel} timelineY={TIMELINE_Y} data-tour="timeline-axis"/>
              </div>

              {/* Timeline Gaps */}
              {gaps.length > 0 && (<TimelineGaps gaps={gaps} timelineY={TIMELINE_Y}/>)}
            ...
            if (!scandal)
                return null;
            return (<ScandalCard key={scandal.id} scandal={scandal} position={position} timelineY={TIMELINE_Y} onClick={() => dispatch({ type: 'SELECT_SCANDAL', payload: scandal.id })} isSelected={state.selectedScandalId === scandal.id}/>);
        })}

              {/* No scandals message */}
              {filteredScandals.length === 0 && (<div className="absolute inset-0 flex items-center justify-center">
        ...
        {/* Stats Panel */}
        <AnimatePresence>
        {state.showStats && (<StatsPanel stats={stats} scandals={filteredScandals} onClose={() => dispatch({ type: 'TOGGLE_STATS' })}/>)}
        </AnimatePresence>
      </div>

      {/* Scandal Details Modal */}
      {selectedScandal && (<ScandalDetails scandal={selectedScandal} onClose={() => dispatch({ type: 'SELECT_SCANDAL', payload: null })}/>)}
