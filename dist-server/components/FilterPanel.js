...
const FilterPanel = ({ filters, onFiltersChange, scandals, onClose }) => {
    ...
    const filterCounts = useMemo(() => {
        ...
        const getCountForFilter = (filterType, filterValue) => {
            return scandals.filter(scandal => {
                ...
                if (tempFilters.personalities.length > 0) {
                    ...
                    const hasPersonality = scandalPersonalities.some(p => tempFilters.personalities.some(fp => p.toLowerCase().includes(fp.toLowerCase())));
                    if (!hasPersonality)
                        return false;
                }
                return true;
    ...
    const handlePersonalityToggle = (person) => {
    ...
    return (<motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'tween', duration: 0.2 }} className="w-80 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col h-full absolute top-0 left-0 z-10 shadow-2xl">
      {/* Header */}
        ...
        */}
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            .map(([type, count]) => (<label key={type} className="flex items-center justify-between cursor-pointer group hover:bg-gray-50 dark:hover:bg-gray-700/50 p-0 rounded-lg transition-colors">
                  <div className="flex items-center gap-3">
                    <div className="relative flex items-center justify-center">
                      <input type="checkbox" checked={filters.types.includes(type)} onChange={() => handleTypeToggle(type)} className="peer sr-only"/>
            ...
            {Object.keys(filterCounts.types).length > ITEMS_PER_SECTION && (<button onClick={() => toggleSection('types')} className="w-full mt-2 flex items-center justify-center gap-1 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors py-2">
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            .map(([party, count]) => (<label key={party} className="flex items-center justify-between cursor-pointer group hover:bg-gray-50 dark:hover:bg-gray-700/50 p-0 rounded-lg transition-colors">
                  <div className="flex items-center gap-3">
                    <div className="relative flex items-center justify-center">
                      <input type="checkbox" checked={filters.parties.includes(party)} onChange={() => handlePartyToggle(party)} className="peer sr-only"/>
                      <div className="h-5 w-5 rounded border-2 border-gray-300 dark:border-gray-600 peer-checked:border-violet-500 dark:peer-checked:border-violet-400 transition-colors"/>
                      <div className="absolute h-3 w-3 rounded-sm bg-violet-500 dark:bg-violet-400 scale-0 peer-checked:scale-100 transition-transform duration-200 ease-out"/>
                    </div>
            ...
            {Object.keys(filterCounts.parties).length > ITEMS_PER_SECTION && (<button onClick={() => toggleSection('parties')} className="w-full mt-2 flex items-center justify-center gap-1 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors py-2">
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            .map(([person, count]) => (<label key={person} className="flex items-center justify-between cursor-pointer group hover:bg-gray-50 dark:hover:bg-gray-700/50 p-0 rounded-lg transition-colors">
                  <div className="flex items-center gap-3">
                    <div className="relative flex items-center justify-center">
                      <input type="checkbox" checked={filters.personalities.includes(person)} onChange={() => handlePersonalityToggle(person)} className="peer sr-only"/>
                      <div className="h-5 w-5 rounded border-2 border-gray-300 dark:border-gray-600 peer-checked:border-violet-500 dark:peer-checked:border-violet-400 transition-colors"/>
                      <div className="absolute h-3 w-3 rounded-sm bg-violet-500 dark:bg-violet-400 scale-0 peer-checked:scale-100 transition-transform duration-200 ease-out"/>
                    </div>
            ...
            {Object.keys(filterCounts.personalities).length > ITEMS_PER_SECTION && (<button onClick={() => toggleSection('personalities')} className="w-full mt-2 flex items-center justify-center gap-1 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors py-2">
      ...
      <div className="p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
        <button onClick={clearFilters} className={`w-full py-2 px-4 rounded-lg text-sm font-medium transition-all ${hasActiveFilters
            ? 'bg-blue-500 hover:bg-blue-600 text-white dark:bg-blue-600 dark:hover:bg-blue-700 shadow-md'
            : 'bg-gray-100 hover:bg-gray-200 text-gray-700 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-300'}`}>
          {hasActiveFilters ? (<div className="flex items-center justify-center gap-2">
